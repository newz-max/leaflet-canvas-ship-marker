...
class CanvasShip extends L.Canvas.CustomCanvas {
  ...
  drawLayer() {
    ...
    this._ctxPoints = res;
    if (!Array.isArray(this._prevEvent))
      this._prevEvent = [];
    this.drawShip(res);
    this._initEvents();
  }
  drawShip(latlngs, point) {
    ...
    latlngs.forEach((item) => {
      ...
      const inRectFn = (ctx2) => {
        return ctx2.isPointInPath(point.x, point.y);
      };
  ...
  _initEvents() {
    ...
    eventItems.forEach((item) => {
      ...
      const currentEvent = lodash.debounce(this.canvasDomEvent.bind(this, eventHandle), 150);
